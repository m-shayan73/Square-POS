      ...
      (set, get) => ({
        ...
        setItems: (items: CartItem[]) => {
          ...
          set({
            items,
            ...(discountsChanged && {
              globalDiscountsApplied: commonDiscounts,
            }),
        ...
        setGlobalDiscounts: (newGlobalDiscounts: Discount[]) => {
          ...
          set({
            globalDiscountsApplied: newGlobalDiscounts,
            items: itemsWithUpdatedDiscounts,
          });
        },
        setGlobalTaxes: (newGlobalTaxes: Tax[]) => {
          ...
          set({
            globalTaxesApplied: newGlobalTaxes,
            items: itemsWithUpdatedTaxes,
          });
        },
        setCartAmounts: ({
          subtotal,
...
const recalculateGlobals = (
  ...
  if (items.length > 0) {
    commonDiscounts = items[0].discountsApplied.filter((discount) =>
      items.every((item) =>
        item.discountsApplied.some(
          (itemDiscount) => itemDiscount.id === discount.id
        )
      )
    );
    ...
    commonTaxes = items[0].taxesApplied.filter((tax) =>
      items.every((item) =>
        item.taxesApplied.some((itemTax) => itemTax.id === tax.id)
      )
    );
  }
  ...
  const discountsChanged =
    ...
    !commonDiscounts.every((d) =>
      currentGlobalDiscounts.some((cd) => cd.id === d.id)
    );
